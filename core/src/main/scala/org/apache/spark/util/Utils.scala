...
private[spark] object Utils extends Logging {
  ...
  def getTachyonPaths(folder: String): List[String] = {
    val predefinedRates = List(0.1, 1.0)
    val predefinedStorageLevel = List("MEMORY_ONLY_SER", "DISK_ONLY", "MEMORY_ONLY")
    implicit class Crossable[X](xs: Traversable[X]) {
      def cross[Y](ys: Traversable[Y]) = for { y <- ys; x <- xs } yield (x, y)
    }
